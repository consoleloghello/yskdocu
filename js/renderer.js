 ...
 function getCurrentQs(){
   var st = State.get();
   var fqs = State.flatQs;
   if(st.mode === 'wrong'){
     return fqs.filter(function(q){ return State.isWrong(q._id); });
   }
   var list = st.searchQuery ? searchIn(fqs, st.searchQuery) :
 ...
 function render(){
   ...
   var qs = getCurrentQs();
    var $ = State.getEl;
   var highlightText = State.highlightText;
 ...
 function renderChapters(){
   var el = State.$('chapterList');
   var st = State.get();
   var chShow = st.mode === 'wrong' ? 'all' : st.chapter;
   var h = '<button class="chip ' + (chShow === 'all' ? 'active' : '') + '" data-ch="all">全部<span class="count">' + State.flatQs.length + '</span></button>';
   State.data.chapters.forEach(function(ch){
     var cnt = ch.type_groups.reduce(function(s,g){ return s + g.questions.length; }, 0);
 ...
 function renderTypeFilters(){
   ...
   var types = countByType(base);
   var order = ['选择题','填空题','判断题','简答题','实操分析题','应急处理题'];
   var sorted = order.filter(function(t){ return types[t]; });
   var total = base.length;
   if(sorted.length <= 1 && st.type === 'all'){ el.style.display = 'none'; return; }
   el.style.display = 'flex';
   var h = '<button class="type-btn ' + (st.type === 'all' ? 'active' : '') + '" data-type="all">全部 <span class="count">' + total + '</span></button>';
 ...
 function renderCards(qs){
   _currentQs = qs;
   var st = State.get();
   var isSearch = st.searchQuery !== '';
   var isLoggedIn = window.SupabaseAuth && window.SupabaseAuth.isLoggedIn();
   var $ = State.$;
   var h = '<div id="listInfo" style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">' +
   ...
   qs.forEach(function(q, idx){
     var isRevealed = State.isRevealed(q._id);
     var isWrong = State.isWrong(q._id);
     var notes = State.getLocalNotes();
     var hasNote = notes[q._id];
     ...
     if(q.options && q.options.length > 0){
       ...
       q.options.forEach(function(opt, oi){
         ...
         var correct = q.answer && q.answer.toUpperCase() === letter;
         var cls = 'opt-row';
 ...
 function updateTopActions(){
   var wc = State.wrongCount();
   State.$('wrongBookBtn').textContent = '❌ 错题 (' + wc + ')';
   var shown = _currentQs.filter(function(q){ return State.isRevealed(q._id); }).length;
   var total = _currentQs.length;
 ...
 window.Render = {
   ...
   getCurrentQs: function(){ return _currentQs; },
   setCurrentQs: function(qs){ _currentQs = qs; },
   logAnswer: logAnswer
 };
 })();
